//Altfe. Daca se introduce in aceste id alt rand care nu este in lista - sa arate eroare inline - sub rand - ca nu este corect randul - sa fie mesajul - Cod rând incorect. Trebuie: 1000-1 (sau alt cod rind corespunzator id-ului)
...
function ensureCap1196ValidationStyles() {
    if (!$("#cap1196InlineValidationStyle").length) {
        $("<style id='cap1196InlineValidationStyle'>" +
            ".cap1196-inline-error{color:#d93025;font-size:11px;margin-top:2px;display:block;}" +
            ".cap1196-invalid{border:1px solid #d93025 !important;background-color:#fff5f5 !important;}" +
          "</style>").appendTo("head");
    }
}

function normalizeCap1196Rind(value) {
    return $.trim(String(value || "")).replace(/\s+/g, "");
}

function getCap1196RindInput(row) {
    return $("#73_1196_" + row.id + "_" + row.rind + "_0");
}

function clearCap1196InlineError($input) {
    if (!$input.length) {
        return;
    }

    var errorId = $input.attr("id") + "_inline_error";
    $input.removeClass("cap1196-invalid");
    $("#" + errorId).remove();
}

function showCap1196InlineError($input, message) {
    if (!$input.length) {
        return;
    }
    ...
    $input.addClass("cap1196-invalid");

    if (!$error.length) {
        $error = $("<span/>", {
            id: errorId,
            "class": "cap1196-inline-error",
            text: message
        });
        $input.after($error);
    } else {
...
function validateCap1196RowCodes(rowList) {
    ensureCap1196ValidationStyles();

    var hasErrors = false;

    rowList.forEach(function (row) {
        var $input = getCap1196RindInput(row);

        if (!$input.length) {
            return;
        }

        var enteredValue = normalizeCap1196Rind($input.val());
        var expectedValue = normalizeCap1196Rind(row.rind);

        if (enteredValue !== "" && enteredValue !== expectedValue) {
            showCap1196InlineError($input, "Cod rând incorect. Trebuie: " + row.rind);
            hasErrors = true;
        } else {
            clearCap1196InlineError($input);
        }
    });
...
function f_Capitol_1196() {
    validateCap1196RowCodes(CAP1196_ROWIDS_1000);
    sumCap1196_Rind1000();
    sumCap1196_Rind1300();
    sumCap1196_Rind1100();
    sumCap1196_Rind1900();
